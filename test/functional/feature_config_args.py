...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_ignored_conf(self):
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir}" contains a "elements.conf" file which is ignored, because a '
        ...
        # error.
        self.start_node(0, [f'-conf={node.datadir}/elements.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        node = self.nodes[0]
        (default_datadir / "elements.conf").write_text(f"datadir={node.datadir}\n")
