...
class CTTest(BitcoinTestFramework):
    ...
    def set_test_params(self):
        ...
        self.extra_args = [
            ...
            args + ["-accept_discounted_ct=0", "-create_discounted_ct=0"],
            # node 1 accepts but does not create discounted CTs
            args + ["-accept_discounted_ct=1", "-create_discounted_ct=0"],
            # node 2 both accepts and creates discounted CTs
            args + ["-accept_discounted_ct=1", "-create_discounted_ct=1"],
        ]
    ...
    def run_test(self):
        ...
        vin = decoded['vin']
        vout = decoded['vout']
        self.log.info(f"vsize: {decoded['vsize']} fee: {tx['fee']}")
        # print(f"vin: {len(vin)} vout: {len(vout)} fee: {tx['fee']}")
        # print(f"weight: {decoded['weight']}, size: {decoded['size']}, vsize: {decoded['vsize']}")
        ...
        for node in [node2, node1]:
            ...
            vin = decoded['vin']
            vout = decoded['vout']
            # print(f"vin: {len(vin)} vout: {len(vout)} fee: {decoded['fee']}")
            # print(f"weight: {decoded['weight']}, size: {decoded['size']}, vsize: {decoded['vsize']}")
            assert_equal(len(vin), 2)
            assert_equal(len(vout), 3)
            if 'bitcoin' in decoded['fee']:
                assert_equal(decoded['fee']['bitcoin'], Decimal('-0.00000381'))
            else:
                assert_equal(decoded['fee'][bitcoin], Decimal('0.00000381'))
            assert_equal(decoded['vsize'], 381)
        # node0 should still report the non-discounted vsize
