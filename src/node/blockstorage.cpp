...
bool BlockManager::LoadBlockIndex(ChainstateManager& chainman)
{
    ...
    if (!m_block_tree_db->LoadBlockIndexGuts(GetConsensus(), [this](const uint256& hash) EXCLUSIVE_LOCKS_REQUIRED(cs_main) { return this->InsertBlockIndex(hash); }, trim_below_height)) {
        return false;
    }
