...
public:
    ...
    bool CheckECDSASignature(const std::vector<unsigned char>& sig, const std::vector<unsigned char>& pubkey, const CScript& script, SigVersion sigversion, unsigned int flags) const override
    {
        if (m_checker.CheckECDSASignature(sig, pubkey, script, sigversion, flags)) {
